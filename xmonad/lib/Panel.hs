...
-- |
-- Module      :  Panel
-- Modified from Mads N Noe 2010 madsnoe.dk
--
...
myConkyConfPath = "/etc/conky/conky_statusbar.conf"
font                  = "-*-terminus-medium-*-*-*-" ++ show myFontSize ++ "-*-*-*-*-*-iso8859-*"
myFontSize              = 12
...
spawnDzenOnScreen count screen = do
    ...
    pipes <- spawnPipe $ dzen
        ...
        (sw -  wTrayerMaybe - wConky) -- horizontal width
        'l'             -- text align
        ""              -- no actions
    spawnDzenWithConky $ dzen
            sy          -- vertical position
            (sx + sw - wConky)  -- horizontal position
            wConky      -- horizontal width
            'r'         -- text align
            ""          -- no actions
    spawn' $ dzen
        sy              -- vertical position
        (sx + sw)  -- horizontal position
        sw	-- horizontal width
        'r'             -- text align
        ""              -- no actions
    return pipes
  ...
  where
    spawnDzenWithConky dest =
        fileExist myConkyConfPath >>=
            (flip when $ do_ $ spawn' $ dzenWithConky myConkyConfPath dest)

    dzenWithConky myConkyConfPath dest = intercalate " " ["conky -c", myConkyConfPath, "|", dest]
