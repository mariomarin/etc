...
-- extra\/haskell-http and aur\/haskell-tagsoup packages.
--
-------------------------------------------------------------------------------
...
formatOutputLines conf rs = concat $ map formatOutputLine rs
...
replaceEntities = replaceAll (namedList ++ numericList) maybeLookup
    ...
    where
        ...
        namedList   = map (("&" ++) . (++";")) $ map fst htmlEntities
        numericList = map (("&#"++) . (++";")) $ map show [1..255]
...
getText t a = concat . map getText' . takeWhile (/= TagClose t) . drop 1 . dropWhile (/= TagOpen t a)
